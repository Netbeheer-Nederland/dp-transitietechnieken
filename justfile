...
initialize:
    ...
    gh repo create Netbeheer-Nederland/dp-transitietechnieken \
        --description "Information models for the Transitietechnieken data product." \
        --public \
        --disable-wiki

    git init -b main

    git remote add origin git@github.com:Netbeheer-Nederland/dp-transitietechnieken.git
    git fetch
    ...
    gh repo edit Netbeheer-Nederland/dp-transitietechnieken --default-branch v0
    git branch -D main
    ...
    #  -H "Accept: application/vnd.github+json" \
    #  -H "X-GitHub-Api-Version: 2022-11-28" \
    #  /repos/Netbeheer-Nederland/dp-transitietechnieken/actions/permissions \
    #   -F "enabled=true" -f "allowed_actions=all"

    echo "… OK."
    echo
    echo "A GitHub repository has been created and configured at: https://github.com/Netbeheer-Nederland/dp-transitietechnieken"
    echo
...
_post-process-linkml-schema:
    ...
    cp "information_models/dp_transitietechnieken.schema.linkml.yml" "artifacts/information_models/"
    @echo
    @echo "Setting version in LinkML schema…"
    @echo
    sed -i '/^version: .*$/d' "artifacts/information_models/dp_transitietechnieken.schema.linkml.yml"
    @if [ -z ${VERSION:-} ]; then \
        sed -i "/^name: .*$/a version: {{shell(ref_name)}}" "artifacts/information_models/dp_transitietechnieken.schema.linkml.yml"; \
    else \
        sed -i "/^name: .*$/a version: ${VERSION}" "artifacts/information_models/dp_transitietechnieken.schema.linkml.yml"; \
    fi
    @echo "… OK."
    @echo
...
[group("schema")]
edit-schema:
    @${VISUAL:-${EDITOR:-nano}} information_models/dp_transitietechnieken.schema.linkml.yml

# Show definition of the class identified by the provided CURIE
[group("schema")]
get-definition curie:
    yq '.classes.* | select(.class_uri == "{{curie}}")' information_models/dp_transitietechnieken.schema.linkml.yml

# List all classes in the schema
[group("schema")]
list-classes:
    yq '.classes.* | key' information_models/dp_transitietechnieken.schema.linkml.yml
...
generate-documentation: _post-process-linkml-schema
    ...
    poetry run python -m linkml_asciidoc_generator.main \
        "artifacts/information_models/dp_transitietechnieken.schema.linkml.yml" \
...
generate-json-schema: _post-process-linkml-schema
    ...
    poetry run gen-json-schema \
        --not-closed \
        "artifacts/information_models/dp_transitietechnieken.schema.linkml.yml" \
        > "artifacts/schemas/json_schema/dp_transitietechnieken.json_schema.json"
    @echo "… OK."
    @echo
    @echo "Generated JSON Schema at: artifacts/schemas/json_schema/dp_transitietechnieken.json_schema.json"
    @echo
...
validate-example-data: generate-json-schema generate-example-data
    ...
    for example_file in artifacts/examples/*.json; do \
        [ -f "$example_file" ] || continue; \
        poetry run check-jsonschema --schemafile "artifacts/schemas/json_schema/dp_transitietechnieken.json_schema.json" $example_file; \
    done
    @echo "… OK."
    @echo
